 ...
 */
public enum TarotLord {
	Magician(new int[][]{{2,4,1,4}, {3,2,1,5}, {1,5,2,2}}),//same as SFC 
	Priestess(new int[][]{{5,1,5,2}, {3,1,4,2}, {2,1,5,1}}), 
	Empress(new int[][]{{3,2,1,4}, {4,1,4,1}, {1,4,3,5}}),//similar to Priestess, wrong here previously as 1,4,5,3  
	...
	Hermit(new int[][]{{3,5,1,4}, {4,1,3,3}, {2,3,4,1}}),//Strength in SFC 
	
	Fortune(new int[][]{{5,1,2,1}, {1,5,2,4}, {2,4,5,3}}),//Hermit in SFC
	Justice(new int[][]{{1,4,4,2}, {4,1,2,5}, {2,3,1,4}}),//Fortune in SFC 
	HangedMan(new int[][]{{1,5,1,3}, {4,2,2,4}, {3,3,4,4}}),//Justice
	
  	Death(new int[][]{{1,2,5,2}, {1,3,2,4}, {5,4,1,4}}),//Hanged Man in SFC 
    ...
    World(new int[][]{{1,1,2,5}, {4,2,4,2}, {2,5,2,1}});//Fool
    
	private int[][] values = new int[4][3];
	
    TarotLord(int [][] values) {
        this.values = values;
    }
    ...
    public static int[][] getValues(TarotLord tarot) {
    	return tarot.values;
    }
    
    public static int[] getValues(TarotLord tarot, int answer) {
