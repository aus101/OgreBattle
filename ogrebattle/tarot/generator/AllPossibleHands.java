 ...
 * The number of cards dealt is the <code>handSize</code> and <code>doNotSort</code> as <code>true</code> is random versus sorted order.<br>
 ...
 * Takes less than 1 second on a mid-tier i5 PC for non-sorted and less than 3 seconds for sorted for hands of 6 cards. Less than 2 seconds<br>
 * and 5 seconds respectively for sorted hand size of 7.<br>
 * Sorting is unnecessary for generating random hands or calculating exact odds. <code>Contains</code> calls on a sorted list vary in time.
 * For instance, only the first sorted 20349 hands contain Magician and hand 35853 is the last with a Priestess.<br>
 * The order for sets of hands is deterministic for HashSet due to bucket hashing regardless of the order they are added/generated.<br>
 * Sorted hands will not work where order matters such as the 7th bonus card. Can choose a random card as the 7th. Lord type only uses 6.<br>
 * Replacing TreeSet for LinkedHashSet would allow permutations, i.e. the order of cards to matter but no data structure with 1 GB RAM is<br>
 * holding 22*21*20*19*18*17 = 53,721,360 permutations. All lord types can be obtained with 98-100% of attempts without considering order.<br>
 */
